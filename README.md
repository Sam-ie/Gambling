...
无需玩家参与，纯 NPC 之间运行 N 回合。支持四种运行模式：

| 模式 | 按钮 | 粒度 | 驱动 | 连线 | 停顿 | 淘汰/折算 |
|------|------|------|------|------|------|-----------|
| **逐对演化** | 开始 | 一对 | 50ms 定时器 | ✓ | 淘汰轮 300ms | ✓ |
| **极速演化** | 极速演化 | 一整轮 | 50ms 定时器 | ✗ | 无 | ✓ |
| **步进** | 步进 | 一对 | 手动点击 | ✓ | 手动控制 | ✓ |
| **单轮** | 单轮 | 一对 | 50ms 自动跑完一轮 | ✓ | 淘汰轮 300ms | ✓ |

**逐对演化（开始按钮）**：
```
每 50ms 触发一次 autoEvoTick()（pair-by-pair 模式）：
  1. 找下一对 NPC (i, j)，执行交互
  2. 发射 autoEvoStep + autoEvoPairDone（连线）
  3. 所有对完成后：
     a. 非淘汰轮 → 直接推进回合
     b. 淘汰轮 → applyElimination() → 置 waiting 标志
        → 定时器切到 300ms（停顿显示真实积分）
        → 下一 tick：adjustScores() → 恢复 50ms
  4. 到达目标 → 停止 → emit autoEvoFinished
```

**极速演化（极速演化按钮）**：
```
每 50ms 触发一次 autoEvoTick()（一轮一帧模式）：
  1. 所有 NPC 两两交互（一整轮）
  2. 淘汰轮直接 eliminate + adjustScores
  3. 发射 autoEvoStep → 目标到达则停止
```

**步进（步进按钮）**：手动点击逐对执行，与逐对演化共享同一套 pair 逻辑，淘汰轮多一步"折算步"需额外点击。

**单轮（单轮按钮）**：定时器驱动逐对跑完一整轮后自动停止。淘汰轮在折算步前停止（显示真实积分）。
...
- 圆心区域放置操作按钮：开始游戏 / 重置（闲置时绿色"开始游戏"，运行中灰色"重置"）
